...
var curityui = (function () {
  ...
  const _qrTimer = function () {
    ...
    if (qrTimeProgress) {
      ...
      const time_indicator_seconds = document.getElementById(
        "qr-time-label-seconds"
      );
  ...
  const validation = (function () {
    ...
    class MinimumSpecial extends MatchingCharacters {
      constructor(minCount) {
        ...
        this.symbolsSet = new Set([
          ...strCodePoints(baseSymbols),
          ...strCodePoints(latin1Symbols),
        ]);
      ...
      isMatch(codePoint) {
        return (
          this.symbolsSet.has(codePoint) ||
          this.currencySymbolsRegExp.test(codePoint)
        );
      }
    }
    ...
    class MaximumSequence {
      ...
      isValid(str) {
        ...
        for (let startIndex = 0; startIndex < stopIndex; startIndex++) {
          const foundInSequence = this.sequences.find((s) =>
            this.isDisallowedSubsequence(codePoints, startIndex, s)
          );
          if (foundInSequence) {
            return false;
          }
      ...
      isDisallowedSubsequence(str, startIndex, sequence) {
        ...
        if (
          sequenceIndex < 0 ||
          sequenceIndex + this.maxCount > sequence.length
        ) {
          return false;
        }
    ...
    function bindInputRequirementsValidation(
      inputElementId,
      rulesElementId,
      rules
    ) {
      ...
      const getRuleElement = function (index) {
        const selector =
          "#" +
          rulesElementId +
          " .input-requirements-rule:nth-child(" +
          (index + 1) +
          ")";
  ...
  function setEnableNonEssentialCookies(enable) {
    if (typeof enable === "boolean") {
      ...
      throw Error(
        "setEnableNonEssentialCookies expects a boolean but got: " + enable
      );
    }
  }
